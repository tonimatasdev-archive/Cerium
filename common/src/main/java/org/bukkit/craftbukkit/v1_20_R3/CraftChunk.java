...
public class CraftChunk implements Chunk {
    ...
    public CraftChunk(ServerLevel ServerLevel, int x, int z) {
    ...
    public IChunkAccess getHandle(ChunkStatus chunkStatus) {
        IChunkAccess chunkAccess = ServerLevel.getChunk(x, z, chunkStatus);
    ...
    public Block getBlock(int x, int y, int z) {
        validateChunkCoordinates(ServerLevel.getMinBuildHeight(), ServerLevel.getMaxBuildHeight(), x, y, z);

        return new CraftBlock(ServerLevel, new BlockPos((this.x << 4) | x, y, (this.z << 4) | z));
    }
    ...
    public BlockState[] getTileEntities() {
        ...
        for (BlockPos position : chunk.blockEntities.keySet()) {
            entities[index++] = ServerLevel.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()).getState();
        }
    ...
    public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        IRegistry<BiomeBase> iregistry = ServerLevel.registryAccess().registryOrThrow(Registries.BIOME);
        ...
        for (int i = 0; i < cs.length; i++) {
            ...
            if (skyLightArray == null) {
                sectionSkyLights[i] = ServerLevel.dimensionType().hasSkyLight() ? FULL_LIGHT : EMPTY_LIGHT;
            } else {
    ...
    public LoadLevel getLoadLevel() {
        net.minecraft.world.level.chunk.Chunk chunk = ServerLevel.getChunkIfLoaded(getX(), getZ());
        if (chunk == null) {
    ...
    public boolean equals(Object o) {
        ...
        return ServerLevel.equals(that.ServerLevel);
    }

    @Override
    public int hashCode() {
        int result = ServerLevel.hashCode();
        result = 31 * result + x;
        result = 31 * result + z;
        return result;
    }
