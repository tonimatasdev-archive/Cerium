...
public class CraftBlockEntityState<T extends BlockEntity> extends CraftBlockState implements TileState {
    ...
    private T createSnapshot(T tileEntity) {
        ...
        CompoundTag CompoundTag = tileEntity.saveWithFullMetadata();
        T snapshot = (T) BlockEntity.loadStatic(getPosition(), getHandle(), CompoundTag);
    ...
    public void loadData(CompoundTag CompoundTag) {
        snapshot.load(CompoundTag);
        load(snapshot);
    }

    // copies the BlockEntity-specific data, retains the position
    private void copyData(T from, T to) {
        CompoundTag CompoundTag = from.saveWithFullMetadata();
        to.load(CompoundTag);
    }
    ...
    public boolean update(boolean force, boolean applyPhysics) {
        ...
        if (result && this.isPlaced()) {
            BlockEntity tile = getBlockEntityFromWorld();
    ...
    public Packet<ClientGamePacketListener> getUpdatePacket(@NotNull Location location) {
        T vanillaTileEntitiy = (T) BlockEntity.loadStatic(CraftLocation.toBlockPos(location), getHandle(), getSnapshotNBT());
